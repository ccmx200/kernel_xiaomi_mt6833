...
/* === ReSukiSU function wrappers/stubs === */

/* Wrapper: ReSukiSU calls susfs_enable_log, old susfs has susfs_set_log */
...
/* Stub: sdcard monitor - not implemented on 4.14 */
void susfs_start_sdcard_monitor_fn(void)
{
    /* No-op stub for ReSukiSU compatibility */
    pr_info("susfs: sdcard monitor stub called\n");
}
EXPORT_SYMBOL(susfs_start_sdcard_monitor_fn);

/* === ReSukiSU extended function stubs === */
/* These functions are stubs for ReSukiSU's extended SUSFS interface */
/* They return success but don't implement full functionality on 4.14 */

#include <linux/uaccess.h>

int susfs_add_sus_path_loop(void __user *arg)
{
    /* Stub: just call susfs_add_sus_path for compatibility */
    struct st_susfs_sus_path info;
    if (copy_from_user(&info, arg, sizeof(info)))
        return -EFAULT;
    return susfs_add_sus_path(&info);
}
...
int susfs_set_hide_sus_mnts_for_non_su_procs(void __user *arg)
{
    ...
    if (copy_from_user(&enabled, arg, sizeof(bool)))
        return -EFAULT;
    /* Stub: accept but no-op on this version */
    return 0;
}
...
{
    char variant[] = "susfs4ksu-4.14-resukisu";
    if (copy_to_user(arg, variant, sizeof(variant)))
        return -EFAULT;
    return 0;
}
...
/* === ReSukiSU compatibility additions === */
#include <linux/sched.h>

struct work_struct susfs_extra_works;
EXPORT_SYMBOL(susfs_extra_works);

#ifndef TIF_KSU_SUSFS_UMOUNTED
#define TIF_KSU_SUSFS_UMOUNTED 24
#endif

bool susfs_is_current_proc_umounted(void)
{
    return test_ti_thread_flag(current_thread_info(), TIF_KSU_SUSFS_UMOUNTED);
}
EXPORT_SYMBOL(susfs_is_current_proc_umounted);

void susfs_set_current_proc_umounted(void)
{
    set_ti_thread_flag(current_thread_info(), TIF_KSU_SUSFS_UMOUNTED);
}
